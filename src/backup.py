...
class Decoder(nn.Module):
    ...
    def forward(self, z: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        bio_factors = self.decoder_net(z['z'])
...
class CrossSpeciesVAE(pl.LightningModule):
    ...
    def _compute_homology_loss(self, outputs: Dict[int, Any], batch: BatchData) -> torch.Tensor:   
        ...
        for target_species_id in outputs:
            ...
            data_to_concat = {target_species_id: batch.data[target_species_id]}
            for species_id in self.species_vocab_sizes.keys():
                if species_id == target_species_id:
                    continue
                
                other_data = batch.data[species_id]
                data_to_concat[species_id] = self.decoders[str(target_species_id)](
                    self.encoders[str(species_id)](other_data)
                )['mean']
            
            data_to_concat = torch.cat([data_to_concat[k] for k in self.species_vocab_sizes.keys()], dim=0)
            assembled_data[target_species_id] = data_to_concat
    ...
    def _compute_reconstruction_loss(
        ...
        for target_species_id in outputs:
            reconstruction = outputs[target_species_id]['reconstructions']
            ...
            for species_id in reconstruction:
    ...
    def _compute_kl_loss(
        ...
        for target_species_id in outputs:
            mu = outputs[target_species_id]['encoder_outputs'][target_species_id]['mu']
            logvar = outputs[target_species_id]['encoder_outputs'][target_species_id]['logvar']
    ...
    def _single_species_forward(self, batch: BatchData):
        ...
        for target_species_id, target_data in batch.data.items():
            ...
            target_encoder_outputs = self.encoders[str(target_species_id)](target_data)
            
            encoder_outputs = {target_species_id: target_encoder_outputs}
            reconstructions = {}
            inputs = {target_species_id: target_data}
            ...
            if self.get_stage() == "transform_recon":
                ...
                for species_id in self.species_vocab_sizes.keys():                
                    reconstructions[species_id] = self.decoders[str(target_species_id)](encoder_outputs[species_id])

            else:
                reconstructions = {target_species_id: self.decoders[str(target_species_id)](target_encoder_outputs)}
    ...
    def get_stage(self): # 0: direct recon loss, 1: transform recon loss with fused latents, 2: homology loss, 3: latent alignment
